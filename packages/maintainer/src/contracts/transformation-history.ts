 ...
 */
export function addMapping(entry: TransformationHistoryEntry, mapping: FactoringMapping): void {
...
export function validateHistory(data: unknown): data is TransformationHistory {
  ...
  for (const entry of history.entries) {
    ...
    if (!['transformed', 'failed', 'partial'].includes(entry.status)) return false;
    ...
    for (const mapping of entry.mappings) {
      ...
      if (!['command', 'agent', 'skill', 'script'].includes(mapping.type)) return false;
    }
  }

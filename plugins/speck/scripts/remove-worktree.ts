...
import type { RemoveWorktreeOptions, RemoveWorktreeOutput } from './contracts/cli-interface';
 ...
 */
async function getWorktreeInfo(worktreePath: string): Promise<WorktreeInfo> {
  ...
  try {
    const branchResult = await $`git -C ${worktreePath} rev-parse --abbrev-ref HEAD`.quiet();
    branchName = branchResult.text().trim();

    if (branchName === 'HEAD') {
      isDetached = true;
      const shortHash = await $`git -C ${worktreePath} rev-parse --short HEAD`.quiet();
...
async function checkGitStatus(
  ...
  const stagedResult = await $`git -C ${worktreePath} diff --cached --name-only`.quiet();
  ...
  const unstagedResult = await $`git -C ${worktreePath} diff --name-only`.quiet();
  ...
  if (!isDetached) {
    try {
      ...
      if (hasUpstream) {
        const unpushedResult = await $`git -C ${worktreePath} rev-list --count @{u}..HEAD`.quiet();
...
async function handleUnpushedCommits(
  ...
  try {
    ...
    console.log('Note: Commits still exist on the branch. You can push them later.');
    return false;
  }
}
  ...
  if (deleteBranch && !isDetached) {
    ...
    try {
      ...
      const otherWorktrees = worktreeList
        .text()
        .split('\n')
        .filter((line) => line.includes(`[${branchName}]`) && !line.includes(worktreePath));
 ...
 */
function outputResults(output: RemoveWorktreeOutput, jsonFormat: boolean): void {
  if (jsonFormat) {
...
async function main(): Promise<void> {
  try {
    ...
    if ((info.branchName === 'main' || info.branchName === 'master') && !options.autoConfirm) {
    ...
    const status = await checkGitStatus(worktreePath, info.branchName, info.isDetached);

    const { hasIssues, warnings } = displayStatus(info, status, options.json || false);
    ...
    if (!options.autoConfirm && !options.json) {
      console.log('Use --auto-confirm to skip this prompt in non-interactive mode');
      console.log('');

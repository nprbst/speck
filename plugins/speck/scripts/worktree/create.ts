...
import type { CreateWorktreeOptions, CreateWorktreeResult, WorktreeMetadata } from './types';
...
export async function createWorktree(
  ...
  try {
    ...
    const worktreePath =
      customWorktreePath || (await constructWorktreePath(repoPath, config.worktree, branchName));
    ...
    if (config.worktree.dependencies.autoInstall && !skipDeps) {
      ...
      if (depsResult.success) {
        progress(
          `Dependencies installed with ${depsResult.packageManager} in ${depsResult.duration}ms`,
          88
        );
      } else {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new GitWorktreeError(`Failed to create worktree: ${errorMessage}`, errorMessage);
  }
}

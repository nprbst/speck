...
async function displayTextOutput(
  config: SpeckConfig,
  context: MultiRepoContextMetadata
): Promise<void> {
 ...
 */
async function displayBranchMappingStatus(
  config: SpeckConfig,
  context: MultiRepoContextMetadata
): Promise<void> {
  ...
  if (
    (context.mode === 'multi-repo' && context.context === 'root') ||
...
async function buildEnvOutputData(
  _config: SpeckConfig,
  context: MultiRepoContextMetadata
): Promise<EnvOutputData> {
  // Get current branch
  let currentBranch = '';
  try {
    currentBranch = await getCurrentBranch(context.repoRoot);
...
async function displayHookOutput(
  config: SpeckConfig,
  context: MultiRepoContextMetadata
): Promise<void> {
  const data = await buildEnvOutputData(config, context);

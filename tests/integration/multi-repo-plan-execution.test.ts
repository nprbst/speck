...
`;
  ...
  test('T102: Commands read shared artifacts from root, write implementation artifacts to child', async () => {
    ...
    await Bun.write(path.join(rootSpecDir, 'research.md'), '# Research\n\nShared research notes.');
  ...
  test('T103: check-prerequisites outputs correct paths in multi-repo mode', async () => {
    ...
    const result =
      await $`cd ${childDir} && bun run ${checkScript} --json --skip-plan-check`.quiet();
    ...
    expect(normalizePath(output.result.IMPL_PLAN)).toBe(
      normalizePath(path.join(childDir, 'specs', featureName, 'plan.md'))
    ); // Points to child
    expect(normalizePath(output.result.TASKS)).toBe(
      normalizePath(path.join(childDir, 'specs', featureName, 'tasks.md'))
    ); // Points to child

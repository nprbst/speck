...
describe('T073: Slash Command Functionality', () => {
  test('/speck.check-upstream command exists', () => {
    const commandPath = join(process.cwd(), '.claude', 'commands', 'speck.check-upstream.md');
    expect(existsSync(commandPath)).toBe(true);
  });

  test('/speck.pull-upstream command exists', () => {
    const commandPath = join(process.cwd(), '.claude', 'commands', 'speck.pull-upstream.md');
    expect(existsSync(commandPath)).toBe(true);
  });

  test('/speck.transform-upstream command exists', () => {
    const commandPath = join(process.cwd(), '.claude', 'commands', 'speck.transform-upstream.md');
    expect(existsSync(commandPath)).toBe(true);
  });

  test('commands reference correct script paths', async () => {
    const checkCommandPath = join(process.cwd(), '.claude', 'commands', 'speck.check-upstream.md');

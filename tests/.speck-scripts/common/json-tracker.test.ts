...
describe('Release Registry Manager', () => {
  ...
  describe('updateStatus', () => {
    test('updates release status to transformed', async () => {
      ...
      const registry = await updateStatus(registryPath, 'v1.0.0', ReleaseStatus.TRANSFORMED);
    ...
    test('updates release status to failed with error details', async () => {
      ...
      expect(registry.releases[0].errorDetails).toBe('Transformation failed: syntax error');

      // Verify persisted to file
      const readBack = await readRegistry(registryPath);
      expect(readBack.releases[0].status).toBe(ReleaseStatus.FAILED);
      expect(readBack.releases[0].errorDetails).toBe('Transformation failed: syntax error');
    });

    test('throws error when version not found', async () => {
      try {
        await updateStatus(registryPath, 'v99.99.99', ReleaseStatus.TRANSFORMED);
        expect(true).toBe(false); // Should not reach here
      } catch (error) {
        expect(error).toBeInstanceOf(Error);

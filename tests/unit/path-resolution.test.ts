...
describe('Path Resolution', () => {
  describe('getRepoRoot()', () => {
    beforeAll(() => {
      // Store current working directory (for test context, not used directly)
      process.cwd();
    });
  ...
  describe('getFeaturePaths()', () => {
    it('should resolve paths relative to current working directory', async () => {
      ...
      // SPECS_DIR should be a valid path string (may or may not exist depending on test environment)
      expect(paths.SPECS_DIR).toBeTruthy();
      expect(typeof paths.SPECS_DIR).toBe('string');
    });

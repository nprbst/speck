...
**Date**: 2025-11-22
**Purpose**: Define feature compatibility table showing which capabilities are supported across repository modes and workflow modes

---

## Overview

The Capability Matrix is a reference table showing feature compatibility across different Speck usage contexts. This contract defines the exact structure, content, and notation used in the matrix, which will be implemented as a documentation page at `/docs/reference/capability-matrix`.
...
**Features** (rows): Core capabilities from specs 007-010

**Additional Column**:
- **Limitations**: Edge cases, constraints, or special considerations
...
When using ⚠️ Partial or providing clarification, include parenthetical note:

**Format**: `⚠️ Partial (brief explanation)`

**Examples**:
- `⚠️ Partial (Added in spec 009)`
- `✅ Supported (via symlink)`
- `N/A (Root defines specs)`
...
**Notation in Matrix**:
- Spec 008 features show `⚠️ Limited (Spec 008)` for Multi-Repo Child column
- Spec 009 features show `✅ Supported (Spec 009)` for Multi-Repo Child column
...
Before publishing capability matrix, verify:

- ✅ All features from specs 007-010 included
- ✅ Support status accurate for each repository mode
- ✅ Limitations column documents known edge cases
- ✅ Notation consistent (✅, ❌, ⚠️, N/A used correctly)
- ✅ Spec progression notes explain 008 → 009 evolution
- ✅ Usage guidelines provide clear decision criteria
- ✅ No contradictions with source specifications

---

## Future Expansion

As new features are added to Speck, the capability matrix should be updated with:

1. New feature rows (e.g., future specs beyond 010)
2. Updated support status (features becoming available in new contexts)
3. Resolved limitations (edge cases addressed in later specs)

**Versioning**: Capability matrix page should show "Last Updated" date and list which specs are covered (e.g., "Features from specs 007-010").

---

**Capability Matrix Version**: 1.0 (covers specs 007-010)

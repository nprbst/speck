...
---
scripts:
  sh: plugins/speck/scripts/setup-plan.ts --json
---
```
...
- Update bash paths: `scripts/bash/X.sh` → `plugins/speck/scripts/X.ts`
...
```yaml
agent_scripts:
  sh: plugins/speck/scripts/update-agent-context.ts __AGENT__
```
...
2. **Update path references**: `scripts/bash/` → `plugins/speck/scripts/`
...
**Command**: `speck.custom.md` **Missing Script**: `plugins/speck/scripts/custom.ts`
...
- Script: `scripts/bash/setup-plan.sh` → `plugins/speck/scripts/setup-plan.ts`
...
- [ ] All bash script refs → `plugins/speck/scripts/*.ts`
